...
const App = () => {
  ...
  const previouslyFocusedTriggerRef = useRef(null);
  ...
  useEffect(() => {
    const contentNode = appContentRef.current;
    if (!contentNode) return;

    if (chordModal || modeModal) {
  ...
  const closeModals = (options = {}) => {
    const { restoreFocus = true } = options;
    setChordModal(null);
    setModeModal(null);
    if (restoreFocus) {
      const trigger = previouslyFocusedTriggerRef.current;
      if (trigger && typeof trigger.focus === "function") {
        trigger.focus();
      }
    }
    previouslyFocusedTriggerRef.current = null;
  };
  ...
  const handleChordClick = (event, chordName, index) => {
    if (!keyInfo) return;
    previouslyFocusedTriggerRef.current = event.currentTarget;
  ...
  const handleModeClick = (event, modeName, index) => {
    if (!keyInfo) return;
    previouslyFocusedTriggerRef.current = event.currentTarget;
                  ...
                  <div id="seventhChords" className="grid gap-4">
                    {keyInfo.seventhChords.map((chord, index) => (
                      <button
                        key={chord}
                        type="button"
                        onClick={(event) => handleChordClick(event, chord, index)}
                  ...
                  <div id="modes" className="grid gap-4">
                    {keyInfo.modes.map((mode, index) => (
                      <button
                        key={mode}
                        type="button"
                        onClick={(event) => handleModeClick(event, mode, index)}
